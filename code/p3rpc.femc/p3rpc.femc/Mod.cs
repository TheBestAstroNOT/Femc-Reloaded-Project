...
namespace p3rpc.femc
{
	...
	public class Mod : ModBase // <= Do not Remove.
	{
		...
		public Mod(ModContext context)
		{
			...
			InitializeModules();
			if (_configuration.frmwtrue == frmw.Ryo)
...
=======
		private void BGMEGenerateMusicScript()
		{
            ...
            try
            {
                ...
                var collection = new Dictionary<string, Tuple<bool, string>>
                {
					...
					{"2011",new Tuple<bool, string>(_configuration.nightwmosq,"night")},
		...
		private void FixBattleThemesMissFix()
		{
            var ryo = GetDependency<IRyoApi>("Ryo");
            if (_configuration.mosqeidk || _configuration.mosq || _configuration.karma || _configuration.rock)
			{
				ryo.AddAudioFolder(Path.Combine(_modLoader.GetDirectoryForModId(_modConfig.ModId), "BGM\\BattleThemesFix"));
			}
		}
